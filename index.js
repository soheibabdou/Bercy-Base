...
async function verifyPayment(txHash) {
  try {
    ...
    const wallet = process.env.BASE_USDC_WALLET.toLowerCase();
...
async function getLiveRates() {
  try {
    const [ecbRes, cgRes] = await Promise.all([
      ...
      fetch("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,avalanche-2,binancecoin,matic-network,chainlink,near,cosmos,ripple,algorand,celo&vs_currencies=usd"),
    ]);
    ...
    const crypto = {
      ...
      SOL: cg["solana"]?.usd, AVAX: cg["avalanche-2"]?.usd,
...
<html><head><title>Bercy Base Pay</title>
<style>
  *{box-sizing:border-box}
  body{background:#0a0a0a;color:#0052FF;font-family:monospace;margin:0;padding:40px 20px;display:flex;justify-content:center}
  .wrap{max-width:700px;width:100%}
  h1{color:#FFFFFF;margin-top:0}
  a{color:#0052FF}
  hr{border:none;border-top:1px solid #222;margin:20px 0}
  .badge{border:1px solid #0052FF;padding:4px 10px;margin:4px;display:inline-block;border-radius:4px;font-size:13px;color:#0052FF}
  .ep{background:#111;padding:10px 14px;margin:6px 0;border-radius:4px;display:block;color:#0052FF}
  .ep:hover{background:#1a1a1a}
  .ep a{color:#FFFFFF;text-decoration:none}
</style>
</head><body><div class="wrap">
<h1>🔵 Bercy Base Pay</h1>
<p>World's first <b>x402 + AC2</b> cross-border payment layer on Base Mainnet (Coinbase L2).</p>
<div>
...
<hr>
<p>🔵 Base Mainnet | Chain 8453 | x402 + AC2<br>
...
app.get("/api/health", async (req, res) => {
  try {
    ...
    res.json({ status: "ok", service: "Bercy Base Pay", network: "Base Mainnet", protocols: ["x402", "AC2"], totalCorridors: 95 });
  }
});
...
app.post("/api/authorize", async (req, res) => {
  ...
  const approval_id = "ac2_base_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8);
  res.json({
    approved: true,
    approval_id,
    from, to, amount,
    ...
    message: "AC2: Human approval granted. Proceed with bercy_pay_base.",
    timestamp: new Date().toISOString()
  });
});
...
app.post("/api/orchestrate", async (req, res) => {
  ...
  if (!payment) return res.status(402).json({
    ...
    pay_to: process.env.BASE_USDC_WALLET,
    message: "Send $0.10 USDC on Base Mainnet, include tx hash as X-PAYMENT header"
  });
  ...
  const tag = "base_" + Date.now().toString(36);
  res.json({
    success: true,
...
app.listen(PORT, () => console.log("Bercy Base Pay running on port " + PORT));
